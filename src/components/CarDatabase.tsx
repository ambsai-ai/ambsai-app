...
export default function CarDatabase() {
  ...
  const car = (database as Record<string, any>)?.[brand]?.[model]?.[engine];
        ...
        {car && (
          <div className="mt-10 bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
            ...
            <ul className="mt-4 space-y-2 text-gray-300">
              {car.problems.map((problem: string, index: number) => (
                <li key={index}>
                  • {problem}
                </li>
              ))}
